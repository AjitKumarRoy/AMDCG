...
import { type PositionApplication as IPositionApplication } from "@/types";
import { updatePositionApplicationStatus } from "../actions";
import toast from "react-hot-toast";

type AppWithId = IPositionApplication & { _id: string };

const ITEMS_PER_PAGE = 10;

export function PositionApplicationList({ applications }: { applications: AppWithId[] }) {
  ...
  const handleStatusChange = (appId: string, newStatus: boolean) => {
    ...
    startTransition(() => {
      ...
      toast.promise(
        updatePositionApplicationStatus(appId, statusString),
        {
  ...
  return (
    <div className="mt-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-lg font-semibold text-white">All Position Applications</h3>
        ...
        <table className="min-w-[800px] w-full divide-y divide-slate-800">
          <thead className="bg-slate-900">
            <tr>
              ...
              <th className="px-6 py-4 text-left border-r border-slate-800 text-amber-400">Highest Qualification</th>
                    ...
                    <td colSpan={5} className="p-4">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                        <div className="space-y-2">
                          ...
                          {app.currentPosition && <p className="flex items-center gap-2 text-slate-400"><Briefcase className="h-4 w-4"/> Currently: {app.currentPosition}</p>}
