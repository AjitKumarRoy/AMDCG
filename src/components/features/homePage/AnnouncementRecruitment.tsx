...
import { type Announcement, type Recruitment } from "@/types";

type AnnouncementWithId = Announcement & { _id: string };
type RecruitmentWithId = Recruitment & { _id: string };
...
export function AnnouncementRecruitment({ announcements, recruitments }: { announcements: AnnouncementWithId[], recruitments: RecruitmentWithId[] }) {
  ...
  const tabs = [
    { name: "Announcements", icon: FaBullhorn, data: sortedAnnouncements, link: "/news-events/announcement"  },
    { name: "Recruitment", icon: FaUserPlus, data: sortedRecruitments, link: "/news-events/recruitment" },
  ];
                  ...
                  tab.data.map((item, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <Link href={`${tab.link}/${item.slug}`} className="text-sm group flex items-start gap-2 sm:gap-4 rounded-lg p-3 transition-colors duration-300 hover:bg-white/10 border border-white/10" target="_blank">
